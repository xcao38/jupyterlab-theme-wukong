...
import { ArrayExt, ArrayIterator, each, filter, find, map, toArray } from '@lumino/algorithm';
...
export class DirListing extends Widget {
    ...
    _startDrag(index, clientX, clientY) {
        ...
        const ft = this._manager.registry.getFileTypeForModel(item);
        const dragImage = this.renderer.createDragImage(source, selectedNames.length, ft);
    ...
    _doRename() {
        ...
        return Private.doRename(nameNode, this._editNode).then(newName => {
            ...
            if (!isValidFileName(newName)) {
                void showErrorMessage('Rename Error', Error(`"${newName}" is not a valid name for a file. ` +
...
(function (DirListing) {
    ...
    class Renderer {
         ...
         *
         */
        updateItemNode(node, model, fileType = DocumentRegistry.defaultTextFileType) {
